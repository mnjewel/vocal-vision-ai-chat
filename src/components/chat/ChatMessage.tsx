...
import { Copy, Check, Trash, MessageSquare, ThumbsUp, ThumbsDown, Heart, Lightbulb } from 'lucide-react';
...
const ChatMessage: React.FC<ChatMessageProps> = ({ 
  ...
  const { id, role, content, timestamp, imageUrl, pending, metadata } = message;
  ...
  useEffect(() => {
    ...
    if (role === 'assistant' && !pending && content.length > 50 && onFollowUpClick) {
      ...
      if (metadata?.searchResults) {
        suggestions.push('Find similar information');
      }
  ...
  }, [role, content, pending, metadata, onFollowUpClick]);
  ...
  const handleReaction = (reaction: string) => {
    const isActive = !activeReactions[reaction];
    setActiveReactions(prev => ({
      ...prev,
      [reaction]: isActive
    }));
    onReaction(id, reaction, isActive);
  };
  ...
  const formatContent = () => {
    ...
    const withCodeBlocks = content.replace(
      /```(\w+)?([\s\S]*?)```/g,
      (_: string, lang: string, code: string) => `<pre class="neural-code relative group"><div class="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity"><button class="copy-code p-1 rounded-md bg-gray-700/30 hover:bg-gray-700/50 text-gray-200" title="Copy code"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg></button></div><code class="language-${lang || 'text'}">${code}</code></pre>`
    );
  ...
  const reactionButtons = [
    { icon: ThumbsUp, name: 'thumbs-up', tooltip: 'Helpful' },
    { icon: ThumbsDown, name: 'thumbs-down', tooltip: 'Not helpful' },
    { icon: Heart, name: 'heart', tooltip: 'Love this' },
    { icon: Lightbulb, name: 'lightbulb', tooltip: 'Insightful' },
  ];
        ...
        <div className="flex-grow">
          ...
          {role === 'assistant' && !pending && (
            <div className="mt-2 flex flex-wrap gap-1">
              {reactionButtons.map(({ icon: Icon, name, tooltip }) => (
                <Button
                  key={name}
                  variant="ghost"
                  size="sm"
                  className={`px-2 py-1 h-auto rounded-full ${activeReactions[name] ? 'bg-primary/10 text-primary' : 'opacity-70 hover:opacity-100'}`}
          ...
          {role === 'assistant' && !pending && followUpSuggestions.length > 0 && onFollowUpClick && (
            <FollowUpSuggestions 
